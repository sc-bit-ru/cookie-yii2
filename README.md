...
Два режима: **banner** (плашка внизу) и **overlay** (закрывает экран, пока не выберут кнопку). Оба законные: после «Только нужные» сайт открыт. Нельзя закрывать сайт за отказ от Метрики.

Это **не юридическая консультация**. Политику ПДн и реестр оператора каждый сайт оформляет сам.
...
1. **Плашка или оверлей**: те же две кнопки. Overlay можно включить на раздел (`oto`, `blog`), не на весь сайт.
...
Полный каркас: [`example/layout.php`](example/layout.php). Оверлей на раздел: [`example/layout-overlay.php`](example/layout-overlay.php).

```php
// Весь этот layout — оверлей
CookieConsentWidget::widget([
...
// Общий layout: плашка везде, оверлей только на /oto и /blog
CookieConsentWidget::widget([
    'privacyUrl' => ['/site/privacy'],
    'cookiesUrl' => ['/site/cookies'],
...
Оверлей не вырезает HTML: текст страницы всегда в ответе. Без JavaScript оверлей скрыт (`display: none` до клика). Яндекс, Google и другие роботы получают плашку, не стену — экран не закрываем, клоакинга нет (тот же контент). Человек без клика страницу не читает — оверлей для раздела, не для всего сайта.
...
7. Overlay: «Только нужные» снимает оверлей, Метрики нет; «Принять все» снимает оверлей и грузит счётчик.
