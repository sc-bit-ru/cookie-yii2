...
Два режима: **banner** (сайт открыт) и **wall** (без согласия экран закрыт). Для закона и SEO безопаснее banner. Wall — осознанный компромисс, см. ниже.
...
1. **Плашка или стена**: banner — кнопки «Только нужные» и «Принять все»; wall — «Отказаться» и «Принять все», без согласия страница закрыта оверлеем.
2. **Cookie `cookieConsent`**: `all`, `necessary` (banner) или `denied` (wall), 365 дней, `SameSite=Lax`, `Secure` на HTTPS.
...
Полный каркас: [`example/layout.php`](example/layout.php). Стена: [`example/layout-wall.php`](example/layout-wall.php).

```php
CookieConsentWidget::widget([
    'mode' => CookieConsentWidget::MODE_WALL, // или MODE_BANNER
    'privacyUrl' => ['/site/privacy'],
    'cookiesUrl' => ['/site/cookies'],
]);
```

В режиме wall политика и `/cookies` стена не закрывает: иначе нельзя прочитать условия до кнопки. Другие пути — в `freePaths`.
...
**Закон.** Ст. 9 152-ФЗ: согласие свободное, конкретное, информированное. «Принять Метрику или сайт не откроется» — согласие уже не вполне свободное. Роскомнадзор ближе к схеме banner: отказался от аналитики — сайт всё равно работает. Wall можно ставить, если оператор сознательно принимает этот риск (например, без статистики сервис не ведёт). Это не запрет пакета и не «так надо по закону». Юрист сайта решает сам.

Информированность стены не ломаем: до «Принять все» можно открыть политику и `/cookies`. Иначе кнопка — не согласие, а тупик.

**SEO.** Нельзя отдавать роботам пустую страницу и людям — полную (клоакинг). Стена — это оверлей поверх **того же HTML**. Текст страницы остаётся в коде, Яндекс и Google его видят. Не вырезайте `$content` из layout до клика и не делайте отдельную заглушку для ботов.

Минус стены для SEO: человек без клика не читает страницу, поведенческие метрики хуже. Для каталога и посадочных обычно лучше banner.

На [web-dk.ru](https://web-dk.ru) стоит banner, не wall.
...
7. Режим wall, инкогнито: контент в HTML есть, экран закрыт, Метрики нет; «Отказаться» — остаётся заглушка; «Принять все» — экран открылся, ушёл `tag.js`.
8. Wall: `/cookies` и политика открываются без стены.
